...
| `printouts.spec.ts`             | `/teachers/printouts` finder (no 100-item select): a pin is present (`unit` / `proximal` / `doctrinal-mastery` depending on the Denver week), book radios move with arrows, search “joy” picks 2 Nephi 2:25 and updates Print tiles / hints. Returning from a sheet via All printouts keeps `?verse=`. Sheets still cover signed-off 2 Nephi tiles + Advanced glyphs and a long verse. Proof PDFs stay on 2 Nephi 2:25. |

...
| `nav.spec.ts`                   | Logo links home from any page. Every AppNav link (Premium, For teachers → printouts, News, How it works) routes to the right place. Every AppFooter link returns 200. Skip link is keyboard-first and jumps to `#main-content`. Mobile menu opens/closes correctly.                                                                                                                                                                                                                                  |
...
| `printouts.spec.ts`             | `/teachers/printouts` finder (no 100-item select): a pin is present (`unit` / `proximal` / `doctrinal-mastery` depending on the Denver week), book filters switch OT/All, search “joy” picks 2 Nephi 2:25. Beginner / Intermediate / Advanced switch a sheet preview; Print tiles / hints and Save PDF follow that level. Returning from a sheet via All printouts keeps `?verse=`. Sheets still cover signed-off 2 Nephi tiles + Advanced glyphs and a long verse. Proof PDFs stay on 2 Nephi 2:25. |
...
| `analytics.spec.ts`             | Homepage includes the live Plausible `pa-*.js` snippet and `window.plausible` is a function.                                                                                                                                                                                                                                                                                                                                                                                                         |

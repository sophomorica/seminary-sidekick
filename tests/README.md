...
| `nav.spec.ts`                   | Logo links home from any page. Every AppNav link (Premium, For teachers → `/teachers`, News, How it works) routes to the right place. Header has no Library item. Footer Discover For teachers → `/teachers`; Class Play keeps `/for-teachers`. Every AppFooter link returns 200. Skip link is keyboard-first and jumps to `#main-content`. Mobile menu opens/closes correctly. |
